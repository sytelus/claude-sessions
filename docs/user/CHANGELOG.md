...
All notable changes to the Claude Code backup tool will be documented here.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [2.0.0] - 2026-01-07 - Major Rewrite: Incremental Backup & Statistics
...
### Added
- **Incremental backup**: Only copies new/changed files based on timestamps
- **`--backup` mode**: Default mode that backs up and converts all sessions
- **`--list` mode**: Shows projects and their backup status
- **Statistics generation**: Creates `stats.html` and `stats.json` with comprehensive analytics
  - Total sessions, messages, tokens
  - Work hours distribution (hourly histogram)
  - Session duration statistics (min/max/avg/median)
...
- All formats (markdown, html, data) generated by default
- Better HTML output with modern styling
- Markdown now includes thinking blocks in collapsible details

### Deprecated
- `claude-start` command (use `claude-extract` for interactive mode)

### Documentation
- Added REQUIREMENTS.md with detailed specification
- Added INVARIANTS.md for validation and testing
